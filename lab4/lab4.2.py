...
def get_max_clique_in_chordal_graph(filename):
    G = initialize_graph(filename)
    lex_bfs_order = lexBFS(G)

    if not checkLexBFS(G, lex_bfs_order):
        raise Exception("LexBFS order is incorrect!")
    ...
    # If graph is chordal, then max clique is max of length of RN + 1, because for each vertex in lex_bfs_order set RN(v) + {v} is a clique.
    result = 0
    for v in lex_bfs_order:
        result = max(result, len(G[v.idx].RN) + 1)

    return result


test_program('maxclique', get_max_clique_in_chordal_graph)
